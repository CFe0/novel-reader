...
**线上访问（任何设备，推荐）**
- 地址：**https://cfe0.github.io/novel-reader/**
- 来源：本项目 GitHub 公开仓库 `CFe0/novel-reader` 通过 GitHub Actions 自动构建并发布到 GitHub Pages（每次 push 自动更新，无需手动部署）。
- 电脑 / 手机 / 平板浏览器直接打开即可阅读“在线书库”中的小说，**无需安装、无需登录**。阅读进度保存在当前设备的浏览器里。

**桌面 / 本地使用**
- 打开本项目目录下的 **`dist/index.html`**（最新构建的单文件程序），用 Chrome 双击即可使用（含本地 TXT 阅读功能）；
- 开发模式：`npm run dev`，浏览器访问 http://localhost:5173。

**局域网书库（电脑共享给同一网络的手机/平板）**
- 把 TXT 小说放入项目目录的 **`局域网书库/`** 文件夹（此书库不会上传 GitHub、仅局域网可见）；
...
- 书架文件夹/分组：文件夹卡片（4 本预览 + 数量）、新建/重命名/删除/排序；
- 批量管理：勾选多本后移动到分组、移除书架（不删除文件）、置顶/取消置顶；
